...
const ChatDetailPage = () => {
  ...
  // 快捷功能按钮 - 根据RAG模式动态调整
  const quickActions = useRAG && user?.id
    ? [
        { id: '1', label: '📚 总结我的学习内容', prompt: '请总结一下我最近学习的内容' },
        { id: '2', label: '💡 推荐学习主题', prompt: '根据我的学习历史，推荐一些新的学习主题' },
      ...
      if (ollamaAvailable) {
        ...
        setTimeout(() => {
          const aiMessage: Message = {
            id: (Date.now() + 1).toString(),
            role: 'assistant',
            content: '抱歉，AI 服务暂时不可用。请确保 Ollama 服务已启动或登录以使用RAG检索功能。',
            timestamp: Date.now()
          }
          setMessages(prev => [...prev, aiMessage])
          setIsLoading(false)
